...
import { authenticate, scrobbleTrack, updateNowPlaying } from './lib/lastfm.ts'
...
const main = async () => {
  // TODO: save to macOS storage
  let sessionKey = SESSION_KEY
  if (!sessionKey) sessionKey = await authenticate()

  startScrobbling(sessionKey)
}
